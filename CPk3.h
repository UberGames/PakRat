...
#define extra_local_file_header_size 16
#define central_directory_file_header_size 46
#define end_central_dir_record_size 22

typedef struct local_file_header { 
	UInt16	version_needed_to_extract; 
	UInt16	general_purpose_bit_flag; 
	UInt16	compression_method; 
	UInt32	last_mod_file_time; 
	UInt32	crc32; 
...
typedef struct extra_local_file_header { 
	UInt32	crc32; 
	UInt32	compressed_size; 
	UInt32	uncompressed_size; 
} extra_local_file_header; 
...
typedef struct end_central_dir_record { 
	...
	UInt16 first_disk; 
	UInt16 total_entries_central_dir_this_disk; 
...
class CPk3: public CFileArchive
{

public:


	CPk3(CFileArchive *inParent);
	CPk3(FSSpec inFileSpec);
	virtual ~CPk3();
...
private:
	...
	// updated by header processing
	UInt32 	_headersCount;			// incremented for each header processes
    string	_entryName;	  	  		// entry name 
    UInt16  _entryCompression;		// compression type
    SInt32  _entryCompressedSize;	  	  		// size of uncompressed data 
    UInt32	_entryExpandedSize;  	// size of compressed data (zero if uncompressed) 
    UInt32	_entryCrc;		  		// crc of uncompressed data 
    UInt32 	_entryDataOffset;	  	// set up by local header, points to data
	UInt32  _entryLocalHeaderOffset; // set up by central header, points to local header
	
    UInt32 	_directoryEndHeaderLocation;
    UInt32	_directoryCentralHeaderLocation;	  	 
	UInt32	_compressionMethodsUsed;

	// tree creation
	void AddEntry(const char *name, UInt32 offset);
	UInt32 FindEndHeader();

	// header parsers
	void process_local_file_header(UInt32 marker, Boolean add, Boolean open, Boolean print);
	void process_central_file_header(UInt32 marker, Boolean add, Boolean open, Boolean print);
	void process_end_central_dir(Boolean print);
	Boolean process_headers(UInt32 count, Boolean add, Boolean open, Boolean print);
	void get_string(Boolean print, int len,  char *s);

/*
	// stream manipulation
	void skip_csize(void);
	void ReadByte(int *x);
	void ReadBits(int bits, int *x);
	void OutByte(int c);
